---
name: voltra-sdk-publish
description: Review, version, and publish the Voltra Node SDK to npm. Use when the user wants to push, publish, release, or version the voltra-node-sdk, or asks to prepare a release, cut a new version, or ship the SDK.
---

# Voltra SDK Publish

End-to-end workflow for reviewing, versioning, and publishing the `voltra-node-sdk` package.

## Prerequisites

- The SDK lives at `voltra-node-sdk/` (sibling to `voltra-private/`).
- Protocol data is generated from `voltra-private/build.ts`, not from the SDK repo.
...
**Automated checks** — run from the SDK root:
